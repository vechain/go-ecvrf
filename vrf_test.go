...
func BenchmarkVRF(b *testing.B) {
	b.Run("secp256k1sha256tai-proving", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _, err := NewSecp256k1Sha256Tai().Prove(esk, alpha)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("secp256k1sha256tai-verifying", func(b *testing.B) {
		...
		_, pi, _ := NewSecp256k1Sha256Tai().Prove(esk, alpha)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := NewSecp256k1Sha256Tai().Verify(&esk.PublicKey, alpha, pi)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("p256sha256tai-proving", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _, err := NewP256Sha256Tai().Prove(sk, alpha)
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	b.Run("p256sha256tai-verifying", func(b *testing.B) {
		...
		_, pi, _ := NewP256Sha256Tai().Prove(sk, alpha)
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, err := NewP256Sha256Tai().Verify(&sk.PublicKey, alpha, pi)
			if err != nil {
				b.Fatal(err)
			}
		}
